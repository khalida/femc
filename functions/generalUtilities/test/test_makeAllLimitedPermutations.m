...
pass1_SJT = isequal(test1_SJT, expectedResult1);
...
if nColumns ~= size(test2, 2)
    error('test_makeAllLimitedPermutations FAILED');
end
if nColumns ~= size(test2_SJT, 2)
    error('test_makeAllLimitedPermutations SJT FAILED');
end
...
test3_SJT = steinhausJohnsonTrotterLimited(4, 2);
expectedResult3 = [ ...
...
if pass1 && pass2 && pass3
    disp('test_makeAllLimitedPermutations PASSED!');
else
    error('test_makeAllLimitedPermutations FAILED');
end

if pass1_SJT && pass2_SJT && pass3_SJT
    disp('test_makeAllLimitedPermutations SJT PASSED!');
else
    error('test_makeAllLimitedPermutations SJT FAILED');
end
