function [ PFEM, EMD, Sim, pars ] = ...
    trainAllFcastsPar( PFEM, EMD, MPC, Sim, all_demand_vals, trControl, k)
...
for instance = 1:numInstances
    ...
    demand_vals_tr_all = all_demand_vals{instance}(tr_idxs);
    ...
    parfor fcType = 1:(nTrainMethods+1)
        if ~strcmp(lossTypesStrings{fcType}, 'fcastFree')
            thisInstancePars{fcType} = ...
                train_FFNN_multStart( demand_vals_tr_all, k,  ...
                lossTypes{min(fcType, nTrainMethods)}, trControl); %#ok<PFBNS>
            thisInstanceTimeTaken(fcType) = toc(tempTic);
        else
            
                        % Train fcastFree model
                        meankWh = mean(demand_vals_tr_all);
            
                        % Seperate training data into initialisation (1-day) and training
                        % (rest)
                        days_init = 1;
                        in_idxs = 1:(days_init*steps_per_day);
                        demand_vals_in = demand_vals_tr_all(in_idxs, :);
                        demand_vals_tr = demand_vals_tr_all(setdiff(tr_idxs, in_idxs), :);
                        ...
                        thisInstancePars{fcType} = genFcastFreeController(allFeatVec,...
