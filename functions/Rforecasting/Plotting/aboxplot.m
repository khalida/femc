...
%         'YData',[0 1 1 0 0]);
    ...
    for i=1:n
        ...
        plot((i-widthl/2+(2*j-1)*widthl/(2*d)).*ones(size(u)),u,...
            'LineStyle','none',...
            'Marker',outMarker,...
...
%         plot((i-widthl/2+(2*j-1)*widthl/(2*d)).*ones(size(ol)),ol,...
        ...
        if q3 > q1
            rectangle('Position',[i-widthl/2+(2*j-1-widths)*widthl/(2*d) q1 widths*widthl/d q3-q1],...
                'EdgeColor','k','FaceColor',myGray,'HitTest','off','Parent',hgg(j));
        end
        ...
        % Plot median marker
%         plot(i-widthl/2+(2*j-1)*widthl/(2*d), q2, markers{j},...
%             'MarkerFaceColor',colors{j,i},'MarkerSize',10,'HitTest',...
%             'off','Parent',hgg(j),'MarkerEdgeColor','k','linewidth',1.2);
...
%         hold off;
    ...
    allMeans(i, j) = u;
    xPosns(i, j) = i-widthl/2+(2*j-1)*widthl/(2*d);
    end
...
legend(fcLabels,'FontSize',myFontSize);
hold off;
